//
//  Info.swift
//  app808
//
//  Created by Dias Atudinov on 28.08.2024.
//

import Foundation

struct Info {
    
    let oneZero: String = "cyberciphersage.fun/app/w1ny0urw4y"
    let appHudID = "app_c4KyQodikFCEN14CiXpoKvGaAZUFq5"
    let metricaID = "9f28cb4b-3479-4780-a789-02a1b6b04fdf"
    let usagePolicy = "https://www.termsfeed.com/live/d8c1e4cc-40b1-4cb4-b602-9b2f4b3a23b1"
    let oneSignal = "a16ff35e-1ff0-426d-a3ed-faac5e5240d6"
    let techCode: String = "scheduled"
    
}
